...
if ( ! function_exists( 'trx_addons_array_search' ) ) {
	 ...
	 */
	function trx_addons_array_search( $arr, $subkey, $value ) {
		foreach ( $arr as $k => $v ) {
...
if ( ! function_exists( 'trx_addons_num2kilo' ) ) {
	/**
	 * Convert number to string with suffix K(ilo)|M(ega)|G(iga)|T(era)|P(enta)
	 * 
...
if ( ! function_exists( 'trx_addons_sc_chat_encode_settings' ) ) {
	/**
...
if ( ! function_exists('trx_addons_prepare_macros') ) {
	/**
	 * Prepare macros in the string: replace {{ and }} on <i> and </i>, (( and )) on <b> and </b>, || on <br>
	 *
