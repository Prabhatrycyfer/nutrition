...
if ( ! function_exists( 'trx_addons_sc_tgenerator_set_total_generated' ) ) {
	function trx_addons_sc_tgenerator_set_total_generated( $number ) {
		$data = get_transient( 'trx_addons_sc_tgenerator_total' );
		...
		$data['per_day'] += $number;
		set_transient( 'trx_addons_sc_tgenerator_total', $data, 24 * 60 * 60 );
	}
}

// Get number of generated tokens from the transient
if ( ! function_exists( 'trx_addons_sc_tgenerator_get_total_generated' ) ) {
	function trx_addons_sc_tgenerator_get_total_generated( $per = 'hour' ) {
		$data = get_transient( 'trx_addons_sc_tgenerator_total' );
		if ( ! is_array( $data ) ) {
			return 0;
		}
...
if ( ! function_exists( 'trx_addons_sc_tgenerator_generate_text' ) ) {
	...
	function trx_addons_sc_tgenerator_generate_text() {
		...
		$limit_per_request = trx_addons_get_option( 'ai_helper_sc_tgenerator_limit_per_request' );
		$limit_per_hour = trx_addons_get_option( 'ai_helper_sc_tgenerator_limit_per_hour' );
		$limit_per_visitor = trx_addons_get_option( 'ai_helper_sc_tgenerator_limit_per_visitor' );
		...
		if ( ! empty( $prompt ) ) {
			...
			$generated_per_hour = trx_addons_sc_tgenerator_get_total_generated();
			$lph = $limit_per_hour < $generated_per_hour + 1;
			...
			if ( OpenAi::instance()->get_api_key() != '' && ! $demo ) {
				...
				trx_addons_sc_tgenerator_set_total_generated( 1 );
