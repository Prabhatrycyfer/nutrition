...
if ( ! function_exists( 'fortius_array_get_first' ) ) {
	/**
	 ...
	 */
	function fortius_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'fortius_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function fortius_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'fortius_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function fortius_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'fortius_array_from_list' ) ) {
	/**
	 ...
	 */
	function fortius_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'fortius_array_slice' ) ) {
	/**
	 ...
	 */
	function fortius_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'fortius_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * fortius_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * fortius_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function fortius_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'fortius_array_insert_after' ) ) {
	/**
	 ...
	 */
	function fortius_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = fortius_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: fortius_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'fortius_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function fortius_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'fortius_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function fortius_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'fortius_generate_id' ) ) {
	/**
	 ...
	 */
	function fortius_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'fortius_get_tag' ) ) {
	/**
	 ...
	 */
	function fortius_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'fortius_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function fortius_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'fortius_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function fortius_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( fortius_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'fortius_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function fortius_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'fortius_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function fortius_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = fortius_fga( $css );
...
if ( ! function_exists( 'fortius_get_column_class' ) ) {
	/**
	 ...
	 */
	function fortius_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'fortius_stripslashes' ) ) {
	/**
	 ...
	 */
	function fortius_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'fortius_get_value_gp' ) ) {
	/**
	 ...
	 */
	function fortius_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'fortius_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function fortius_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'fortius_get_value_gps' ) ) {
	/**
	 ...
	 */
	function fortius_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'fortius_get_session_value' ) ) {
	/**
	 ...
	 */
	function fortius_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'fortius_set_session_value' ) ) {
	/**
	 ...
	 */
	function fortius_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'fortius_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function fortius_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'fortius_get_cookie' ) ) {
	/**
	 ...
	 */
	function fortius_get_cookie( $name, $defa = '' ) {
		return fortius_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'fortius_set_cookie' ) ) {
	/**
	 ...
	 */
	function fortius_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'fortius_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//fortius_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//fortius_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'fortius' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'fortius_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'fortius_hex2rgba' ) ) {
	/**
	 ...
	 */
	function fortius_hex2rgba( $hex, $alpha ) {
		$rgb = fortius_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'fortius_hex2hsb' ) ) {
	/**
	 ...
	 */
	function fortius_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = fortius_rgb2hsb( fortius_hex2rgb( $hex ) );
...
if ( ! function_exists( 'fortius_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'fortius_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'fortius_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'fortius_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function fortius_hsb2hex( $hsb ) {
		return fortius_rgb2hex( fortius_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'fortius_date_to_sql' ) ) {
	/**
	 ...
	 */
	function fortius_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'fortius_format_price' ) ) {
	/**
	 ...
	 */
	function fortius_format_price( $price ) {
...
if ( ! function_exists( 'fortius_num2size' ) ) {
	/**
	 ...
	 */
	function fortius_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'fortius_size2num' ) ) {
	/**
	 ...
	 */
	function fortius_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'fortius_parse_num' ) ) {
	/**
	 ...
	 */
	function fortius_parse_num( $str ) {
...
if ( ! function_exists( 'fortius_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function fortius_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'fortius_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function fortius_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'fortius_is_on' ) ) {
	/**
...
if ( ! function_exists( 'fortius_is_off' ) ) {
	/**
...
if ( ! function_exists( 'fortius_is_inherit' ) ) {
	/**
	 ...
	 */
	function fortius_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'fortius_strshort' ) ) {
	/**
	 ...
	 */
	function fortius_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = fortius_strip_tags( $str );
...
if ( ! function_exists( 'fortius_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function fortius_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'fortius_strip_tags' ) ) {
	/**
	 ...
	 */
	function fortius_strip_tags( $str ) {
...
if ( ! function_exists( 'fortius_excerpt' ) ) {
	/**
	 ...
	 */
	function fortius_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return fortius_strwords( fortius_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'fortius_unserialize' ) ) {
	/**
	 ...
	 */
	function fortius_unserialize( $str ) {
...
if ( ! function_exists( 'fortius_str_replace' ) ) {
	/**
	 ...
	 */
	function fortius_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = fortius_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( fortius_str_replace( $from, $to, fortius_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'fortius_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function fortius_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'fortius_tags_count' ) ) {
	/**
	 ...
	 */
	function fortius_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'fortius_getimagesize' ) ) {
	/**
	 ...
	 */
	function fortius_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = fortius_is_url( $url ) ? fortius_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'fortius_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function fortius_clear_thumb_size( $url, $remove_protocol = true ) {
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = fortius_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'fortius_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function fortius_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = fortius_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = fortius_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = fortius_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = fortius_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'fortius_get_thumb_size' ) ) {
	 ...
	 */
	function fortius_get_thumb_size( $ts ) {
		$retina = fortius_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'fortius_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'fortius_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'fortius-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'fortius_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'fortius-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'fortius_get_closest_thumb_size' ) ) {
	 ...
	 */
	function fortius_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'fortius_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function fortius_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = fortius_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'fortius_get_post_image' ) ) {
	/**
	 ...
	 */
	function fortius_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'fortius_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function fortius_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = fortius_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = fortius_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = fortius_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( fortius_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = fortius_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'fortius_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function fortius_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'fortius_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'fortius_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function fortius_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = fortius_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = fortius_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'fortius_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'fortius_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function fortius_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'fortius_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function fortius_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'fortius_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function fortius_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = fortius_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = fortius_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'fortius_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'fortius_get_embed_video' ) ) {
	/**
	 ...
	 */
	function fortius_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = fortius_make_video_autoplay( $embed_video );
		} else if ( fortius_is_from_uploads( $video ) ) {
			...
			$video = fortius_add_to_url( $video, fortius_get_embed_video_parameters() );
			$tag   = fortius_get_embed_video_tag_name();
			// Calc video width and height (with ratio 16:9)
			$ratio  = explode( ':', apply_filters( 'fortius_filter_video_ratio', '16:9' ) );
			$width  = fortius_get_content_width();
			$height = round( $width / $ratio[0] * $ratio[1] );
			$dim    = apply_filters( 'fortius_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									) );
...
if ( ! function_exists( 'fortius_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function fortius_get_embed_video_tag_name() {
...
if ( ! function_exists( 'fortius_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function fortius_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = fortius_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										fortius_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'fortius_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function fortius_is_from_uploads( $url ) {
		$url          = fortius_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = fortius_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'fortius_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function fortius_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'fortius_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function fortius_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'fortius_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'fortius_init_filesystem', 0 );
	/**
	 ...
	 */
	function fortius_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'fortius_fpc' ) ) {
	/**
	 ...
	 */
	function fortius_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'fortius' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'fortius_fgc' ) ) {
	/**
	...
	function fortius_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? fortius_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'fortius' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'fortius_fga' ) ) {
	/**
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'fortius' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'fortius_mkdir' ) ) {
	/**
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'fortius' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'fortius' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'fortius_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function fortius_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fortius_is_on( fortius_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'fortius' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'fortius_copy' ) ) {
	/**
	...
	function fortius_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = fortius_prepare_path( $src );
				$dst = fortius_prepare_path( $dst );
				...
				if ( is_dir( $src ) ) {
					...
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							fortius_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'fortius_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'fortius_esc' ) ) {
	 ...
	 */
	function fortius_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'fortius_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'fortius_check_min_file' ) ) {
	 ...
	 */
	function fortius_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && fortius_is_off( fortius_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && fortius_is_off( fortius_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'fortius_get_file_dir' ) ) {
	/**
	 ...
	 */
	function fortius_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! FORTIUS_ALLOW_SKINS && ! fortius_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'fortius_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( FORTIUS_CHILD_DIR != FORTIUS_THEME_DIR && file_exists( FORTIUS_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? FORTIUS_CHILD_URL : FORTIUS_CHILD_DIR ) . fortius_check_min_file( $file, FORTIUS_CHILD_DIR );
			} elseif ( file_exists( FORTIUS_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? FORTIUS_THEME_URL : FORTIUS_THEME_DIR ) . fortius_check_min_file( $file, FORTIUS_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'fortius_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function fortius_get_file_url( $file ) {
		return fortius_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'fortius_get_file_ext' ) ) {
	/**
	 ...
	 */
	function fortius_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'fortius_get_file_name' ) ) {
	/**
	 ...
	 */
	function fortius_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'fortius_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function fortius_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'fortius_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( FORTIUS_CHILD_DIR != FORTIUS_THEME_DIR && is_dir( FORTIUS_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? FORTIUS_CHILD_URL : FORTIUS_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( FORTIUS_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? FORTIUS_THEME_URL : FORTIUS_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'fortius_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'fortius_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function fortius_get_folder_url( $folder ) {
		return fortius_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'fortius_merge_js' ) ) {
	/**
	 ...
	 */
	function fortius_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= fortius_fgc( fortius_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = fortius_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = fortius_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			fortius_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'fortius' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'fortius_filter_js_output', apply_filters( 'fortius_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'fortius_merge_css' ) ) {
	/**
	 ...
	 */
	function fortius_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'fortius_filter_responsive_sizes', fortius_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = fortius_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = fortius_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'fortius_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'fortius_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'fortius' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'fortius' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = fortius_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			fortius_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'fortius') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'fortius_filter_css_output', apply_filters( 'fortius_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'fortius_filter_merge_css' ) ) {
	add_filter( 'fortius_filter_merge_css', 'fortius_filter_merge_css' );
	/**
	 ...
	 */
	function fortius_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'fortius_merge_sass' ) ) {
	/**
	 ...
	 */
	function fortius_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'fortius_filter_responsive_sizes', fortius_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'fortius_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			fortius_fpc( fortius_get_file_dir( $to ), apply_filters( 'fortius_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'fortius_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function fortius_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'fortius_add_protocol' ) ) {
	/**
	 ...
	 */
	function fortius_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? fortius_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'fortius_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function fortius_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'fortius_add_to_url' ) ) {
	/**
	 ...
	 */
	function fortius_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = fortius_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'fortius_is_url' ) ) {
	/**
	 ...
	 */
	function fortius_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

if ( ! function_exists( 'fortius_is_external_url' ) ) {
	/**
	 ...
	 */
	function fortius_is_external_url( $url ) {
		return fortius_is_url( $url ) && strpos( $url, fortius_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}

if ( ! function_exists( 'fortius_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function fortius_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => fortius_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => FORTIUS_CHILD_DIR,
				'url' => fortius_remove_protocol_from_url( FORTIUS_CHILD_URL )
				),
			'theme' => array(
				'dir' => FORTIUS_THEME_DIR,
				'url' => fortius_remove_protocol_from_url( FORTIUS_THEME_URL )
				)
			);
