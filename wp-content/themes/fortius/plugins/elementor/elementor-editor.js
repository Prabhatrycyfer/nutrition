...
jQuery( document ).ready(
	function() {
		...
		fortius_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="fortius_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.fortius_post_editor' );
						if ( link.length > 0 ) {
							fortius_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.fortius_post_editor', function(e) {
						fortius_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('fortius_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function fortius_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
