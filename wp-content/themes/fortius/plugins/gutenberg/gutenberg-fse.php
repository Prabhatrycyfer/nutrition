...
if ( ! defined( 'FORTIUS_FSE_TEMPLATE_PART_PT' ) ) define( 'FORTIUS_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'FORTIUS_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'FORTIUS_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'FORTIUS_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'FORTIUS_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'FORTIUS_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'FORTIUS_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'FORTIUS_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'FORTIUS_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once fortius_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once fortius_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'fortius_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'fortius_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function fortius_gutenberg_fse_loader() {
		// 
		if ( fortius_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& fortius_is_on( fortius_get_theme_option( 'front_page_enabled', false ) )
			&& fortius_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'fortius_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function fortius_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'fortius_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function fortius_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'fortius_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function fortius_exists_gutenberg_fse() {
		return fortius_exists_gutenberg() && ( fortius_gutenberg_is_fse_wp_support() || fortius_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'fortius_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function fortius_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( fortius_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( fortius_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( FORTIUS_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( FORTIUS_THEME_DIR != FORTIUS_CHILD_DIR && is_readable( FORTIUS_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( FORTIUS_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( FORTIUS_THEME_DIR != FORTIUS_CHILD_DIR && is_readable( FORTIUS_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'fortius_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function fortius_gutenberg_is_fse_enabled() {
		return fortius_exists_gutenberg_fse() && fortius_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'fortius_gutenberg_fse_skin_updated' ) ) {
	add_action( 'fortius_action_theme_updated', 'fortius_gutenberg_fse_skin_updated' );
	add_action( 'fortius_action_skin_updated', 'fortius_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'fortius_action_skin_updated', 'fortius_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function fortius_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'fortius_action_theme_updated' && empty( $skin ) ) {
			$skin = fortius_skins_get_active_skin_name();
		}
		if ( fortius_skins_get_current_skin_name() == $skin ) {
			fortius_gutenberg_fse_copy_from_skin( $skin );
			fortius_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'fortius_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'fortius_action_skin_switched', 'fortius_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'fortius_action_skin_switched', 'fortius_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function fortius_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = fortius_prepare_path( FORTIUS_THEME_DIR . ( fortius_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = fortius_prepare_path( FORTIUS_THEME_DIR . ( fortius_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = fortius_prepare_path( FORTIUS_THEME_DIR . 'theme.json' );
		$skin_templates_dir = fortius_prepare_path( FORTIUS_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( fortius_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = fortius_prepare_path( FORTIUS_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( fortius_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			fortius_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			fortius_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			fortius_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			fortius_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			fortius_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'fortius_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'fortius_action_skin_switched', 'fortius_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'fortius_action_save_options', 'fortius_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'fortius_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'fortius_action_skin_switched', 'fortius_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'fortius_action_save_options', 'fortius_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'fortius_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'fortius_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function fortius_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'fortius_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = fortius_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( FORTIUS_THEME_DIR . fortius_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = fortius_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				fortius_fpc( fortius_prepare_path( FORTIUS_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'fortius_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'fortius_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function fortius_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'fortius' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'fortius' ),
							"area"  => FORTIUS_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'fortius' ),
							"area"  => FORTIUS_FSE_TEMPLATE_PART_AREA_SIDEBAR,
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'fortius' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'fortius' ),
...
*/
		...
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'fortius' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_dark'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'fortius' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'fortius' )
			);
		}
		...
		if ( ! empty( $scheme['text_hover'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'fortius' ),
			);
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Diagonal from link color to hover color', 'fortius' ),
			);
		}
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'fortius_filter_gutenberg_fse_theme_json_data', $data );
	}
}
