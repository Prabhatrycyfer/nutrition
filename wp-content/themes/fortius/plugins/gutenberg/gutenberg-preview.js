...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			fortius_create_observer( 'fortius-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				fortius_gutenberg_editor_init();
			} );
		} else {
		 	fortius_gutenberg_editor_init();
		}
	...
	function fortius_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'fortius_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'fortius_inited' ) ) {
	...
	// Init on page load
	function fortius_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = fortius_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + FORTIUS_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + FORTIUS_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in FORTIUS_STORAGE['override_classes'] ) {
				$obj.addClass( FORTIUS_STORAGE['override_classes'][i].replace( '%s', FORTIUS_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( fortius_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && FORTIUS_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'fortius_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			fortius_remove_observer( 'fortius-check-editor-styles-wrapper' );
			fortius_create_observer( 'fortius-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'fortius_inited' );
	}
} );
