...
if ( function_exists( 'fortius_elementor_is_preview' ) && fortius_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			fortius_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	fortius_new_main_query(
		array(
			'post_type'      => fortius_get_theme_option( 'post_type' ),
			'category'       => fortius_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'fortius_filter_get_template_part', fortius_blog_archive_get_template() ) );
}
