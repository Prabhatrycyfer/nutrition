<div class="front_page_section front_page_section_testimonials<?php
	$fortius_scheme = fortius_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $fortius_scheme ) && ! fortius_is_inherit( $fortius_scheme ) ) {
		echo ' scheme_' . esc_attr( $fortius_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( fortius_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( fortius_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$fortius_css      = '';
		$fortius_bg_image = fortius_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$fortius_anchor_icon = fortius_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$fortius_anchor_text = fortius_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( fortius_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$fortius_css      = '';
			$fortius_bg_mask  = fortius_get_theme_option( 'front_page_testimonials_bg_mask' );
			$fortius_bg_color_type = fortius_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $fortius_bg_color_type ) {
				$fortius_bg_color = fortius_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$fortius_caption = fortius_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $fortius_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $fortius_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $fortius_caption, 'fortius_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$fortius_description = fortius_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $fortius_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $fortius_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $fortius_description ), 'fortius_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! fortius_exists_trx_addons() ) {
						...
						fortius_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
